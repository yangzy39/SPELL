...
from verl.trainer.ppo.metric_utils import (
    compute_spr_data_metrics,
    compute_throughout_metrics,
    compute_timing_metrics,
    process_spr_validation_metrics,
)
...
class RayPPOTrainer:
    ...
    def _dump_generations(self, inputs, outputs, scores, data_sources, reward_models, reward_extra_infos_dict, dump_path):
    ...
    def _validate(self):
        ...
        #     print(key_info,len(lst))

        for key_info, lst in reward_extra_infos_dict.items():
            assert len(lst) == 0 or len(lst) == len(sample_scores), f"{key_info}: {len(lst)=}, {len(sample_scores)=}"

        def _compute_global_mean_by_n(data_src2var2metric2val, core_var="reward"):
            """
            Args:
                data_src2var2metric2val: dict[data_source] -> var_name -> metric_name -> value
                core_var: which variable to aggregate (e.g., 'reward', 'acc')

            Returns:
                Dict[str, float]: {f"mean@{n}": weighted_avg_value}
            """
            n_to_values = defaultdict(list)
            n_to_counts = defaultdict(int)
            ...
            for data_source, var2metric2val in data_src2var2metric2val.items():
                ...
                for metric_name, value in metric2val.items():
                    if metric_name.startswith("mean@"):
                        try:
                            n = int(metric_name.split("@")[1].split("/")[0])
                        except (IndexError, ValueError):
                            continue

                        # Assume sample_count is available as a field; fallback to 1 if not
                        sample_count = var2metric2val.get('sample_count', 1)
                        n_to_values[n].append((value, sample_count))
            
            result = {}
            for n, vals_and_counts in n_to_values.items():
                total_weight = sum(count for _, count in vals_and_counts)
                weighted_sum = sum(val * count for val, count in vals_and_counts)
                if total_weight > 0:
                    result[f"mean@{n}"] = weighted_sum / total_weight
                else:
                    result[f"mean@{n}"] = float('nan')
            
            return result   

        data_src2var2metric2val = process_spr_validation_metrics(data_sources, sample_inputs, reward_extra_infos_dict, group_prefix = True)
        metric_dict = {}
        for data_source, var2metric2val in data_src2var2metric2val.items():
        ...
        # Then add global mean@n across all data sources
        # global_means = _compute_global_mean_by_n(data_src2var2metric2val, core_var="reward")
        # for key, value in global_means.items():
        #     metric_dict[f"val-core/all/reward/{key}"] = value
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # collect metrics
                metrics.update(compute_spr_data_metrics(batch=batch, use_critic=self.use_critic))
