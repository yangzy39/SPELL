...
class MegatronLoader(BaseModelLoader):
    ...
    def _get_weights_iterator(actor_model: Union[PreTrainedModel, Dict]):
        # NOTE Load the weights from the actor model
        pass
    ...
    def load_model(
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            # TODO: This is a hack, we need to register the load_weight() func for each model in vllm
            if isinstance(actor_model, nn.Module):
        ...
        # NOTE Some weights are point to gpu, but still need this.
        model = model.cuda()  # NOTWe need this for vllm to profile memory usage
        return model.eval()
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            for _, module in model.named_modules():
                ...
                if hasattr(module, "process_weights_after_loading"):
                    module.process_weights_after_loading()
        # NOTE Some weights are point to gpu, but still need this.
        model = model.cuda()  # N We need this for vllm to profile memory usage
        return model.eval()


# FIXME: hack the _get_logits function in vllm v0.4.2
