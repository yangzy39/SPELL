...
# NOTE: replace the origin weight loader function in the class
...
def gpt2_weight_loader(actor_weights: Dict, vllm_model: nn.Module) -> nn.Module:
    ...
    for name, loaded_weight in actor_weights.items():
        ...
        # Note: the logic below might break quantized models.
...
def llama_megatron_weight_loader(actor_weights: Dict, vllm_model: nn.Module) -> nn.Module:
    # NOTE: the megatron llama may have this prefix
...
__LAYER_WEIGHT_MEGATRON_LOADER_REGISTRY__ = {
    ...
    # "ScaledActivation.weight_loader": ScaledActivation, # TODO: latest commit in vllm fix awq for this function and add load_weights
...
def load_megatron_weights(actor_weights: Iterable, vllm_model: nn.Module):
    ...
    # NOTE to reduce peak memory usage, we offload vllm model to cpu
