...
def _set_envs_and_config(server_args: ServerArgs):
    ...
    if server_args.attention_backend == "flashinfer":
        assert_pkg_version(
            "flashinfer_python",
            "0.2.5",
            "Please uninstall the old version and reinstall the latest version.",
        )
    # if is_cuda():
    #     assert_pkg_version(
    #         "sgl-kernel",
...
# NOTE: add for verl. We can optimize it by making
...
class SGLangRollout(BaseRollout):
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        if first_rank_in_node:
            ...
            self._engine = AsyncEngine(
                ...
                # NOTE: add rank to prevent SGLang generate same port inside PortArgs.init_new
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        n_rollouts = prompts.meta_info.get("n", None)
        ...
        elif n_rollouts is not None:
                request_sampling_params.update(
                {
                    "n": n_rollouts,  # if validate, already repeat in ray_trainer
                }
            )
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        do_sample = prompts.meta_info.get("do_sample", True)
        is_validate = prompts.meta_info.get("validate", False)
        n_rollouts = prompts.meta_info.get("n", -9999)
        ...
        if self._tp_rank == 0:
            ...
            output_req_list = loop.run_until_complete(
                asyncio.gather(
                    *[self._async_rollout_a_request(req, do_sample, is_validate, n_rollouts,  **kwargs) for req in req_list],
                )
            )
